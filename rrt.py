...
def isRobotCollided(map, q):
    ...
    """
    obstacles = map.obstacles  # Extracting obstacles from the map structure
...
def isPathCollided(map, q1, q2):
    ...
    for i in range(n_steps):
        ...
        if isRobotCollided(map, q):  # Checking if the interpolated point is collided
...
def rrt(map, start, goal, N_samples=4321):
    """
    ...
    if isRobotCollided(map, start) or isRobotCollided(map, goal):
        return np.array([])
    if not isPathCollided(map, start, goal):
    ...
    for _ in range(N_samples):
        ...
        if isPathCollided(map, nearest_node['config'], q_rand):
            continue
        ...
        if not isPathCollided(map, q_rand, goal):
